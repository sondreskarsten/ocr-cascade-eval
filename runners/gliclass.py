...
def main():
    ...
    if not hasattr(GLiClassModel, "_supports_sdpa"):
        GLiClassModel._supports_sdpa = False
    if not hasattr(GLiClassModel, "_supports_flash_attn_2"):
        GLiClassModel._supports_flash_attn_2 = False
    ...
    def per_pdf(pdf_id, b):
        ...
        for ln in b["full_text"].splitlines():
            ln = ln.strip()
            if 4 <= len(ln) <= 60 and not ln.replace(" ", "").isdigit() and ln and ln[0].isupper():
